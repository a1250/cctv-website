...
const { options } = PLANNER_STRINGS.steps.propertyType;

export default function Step2_PropertyType({ value, onChange }: Props) {
  return (
    <div
      ...
      {options.map((opt, i) => {
        ...
        return (
          <button
            key={opt.value}
            type="button"
            onClick={() => onChange(opt.value as PropertyType)}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 16,
